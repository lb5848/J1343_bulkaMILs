...
workingDir <- "210113_WorkingDirectory"
...
# 
fwrite(list(fileName), "sample_metadata1.csv")
# manually set sample_metadata.csv
...
sample_md[, patient_id := factor(patient_id)]
sample_md[, condition := factor(condition, 
                                levels = c("CR", "PD"))]
levels(sample_md$condition)
levels(sample_md$sample_id)
colnames(sample_md)
fwrite(sample_md, "sample_metadata.csv")

# create panel_md.csv
fwrite(list(colnames(flowSet)), "panel_md_1.csv")
...
keyword(flowSet, "FILENAME")

sce <- CATALYST::prepData(flowSet, panel = panel_md, md = sample_md, transform = FALSE)
...
saveRDS(sce, file = "SCE_CRvsPD.rds")
